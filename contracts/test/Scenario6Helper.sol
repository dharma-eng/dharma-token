...
contract Scenario6Helper {
  ...
  function phaseTwo(
    ...
    // ensure dToken appreciation is at least 90% of expected amount.
    require(
      (
        interestEarnedFromDTokens.mul(_SCALING_FACTOR)
      ).div(
        ninetyPercentOfInterestEarnedFromCTokens
      ) >= _SCALING_FACTOR.sub(1e17),
      "Interest earned on dTokens is < 90% of expected."
    );
  }
}